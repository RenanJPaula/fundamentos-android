...
public class ServiceOrderListActivity extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        final boolean menuShareVisible = mServiceOrdersAdapter != null && mServiceOrdersAdapter.getItemCount() > 0;
        menuShare.setEnabled(menuShareVisible).setVisible(menuShareVisible);
        return super.onPrepareOptionsMenu(menu);
    }

    public class GetServiceOrdersTask extends AsyncTask<Void, Void, List<ServiceOrder>>{

        @Override
        protected List<ServiceOrder> doInBackground(Void... params) {
            return ServiceOrderService.getAll();
        }

        @Override
        protected void onPostExecute(List<ServiceOrder> serviceOrders) {
